CREATE OR REPLACE FUNCTION public.transfer_funds_atomic(p_source_account_id uuid, p_destination_account_id uuid, p_amount numeric, p_profile_id uuid, p_purpose text, p_source text, p_is_transfer boolean DEFAULT false, p_project_id uuid DEFAULT NULL::uuid, p_member_id uuid DEFAULT NULL::uuid, p_payment_method text DEFAULT NULL::text, p_pledge_id uuid DEFAULT NULL::uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
    ...
    IF p_destination_account_id IS NOT NULL THEN
        ...
        -- Record income transaction, including pledge_id if applicable
        INSERT INTO public.income_transactions (profile_id, account_id, amount, source, date, pledge_id)
        VALUES (p_profile_id, p_destination_account_id, p_amount, p_source, NOW(), p_pledge_id);
    END IF;
